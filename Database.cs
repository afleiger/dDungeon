...
namespace Chrisistheon
{
    public class Database
    {
        ...
        public virtual void connect()
        {
            ...
            SQLiteCommand comm = new SQLiteCommand("CREATE TABLE IF NOT EXISTS HeroStats (id INTEGER, class TEXT, hp DOUBLE, power INTEGER, defense INTEGER, speed INTEGER)");
            comm.Connection = con;
            comm.ExecuteNonQuery();
            comm.CommandText = "CREATE TABLE IF NOT EXISTS MonsterStats (id INTEGER, class TEXT, hp DOUBLE, power INTEGER, defense INTEGER, speed INTEGER)";
            comm.ExecuteNonQuery();
            comm.CommandText = "CREATE TABLE IF NOT EXISTS RoomText (id INTEGER, string TEXT)";
            comm.ExecuteNonQuery();
            con.Close();
        }
        ...
        public virtual void addRoomTouple(string text)
        {
            SQLiteConnection con = new SQLiteConnection("Data Source=stats.db;Version=3;");
            con.Open();
            SQLiteCommand comm = new SQLiteCommand("INSERT INTO RoomText values(" + textId + ", '" + text + "')");
            textId++;
            comm.Connection = con;
            comm.ExecuteNonQuery();
            con.Close();
